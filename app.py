...
if "init_admin" not in st.session_state:
    st.session_state.init_admin = False

init_admin_page = st.Page(init_admin, title="初始化管理员")
pg = st.navigation([init_admin_page])
if st.session_state.init_admin:
    login_page = st.Page(login_hash, title="登录")
