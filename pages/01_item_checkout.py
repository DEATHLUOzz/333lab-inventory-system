...
if"item_cheackout_inputid"  not in st.session_state:
    st.session_state.item_checkout_inputid = ""
    st.session_state.item_checkout_inputid = st.session_state.get("goto_item_id", "")
item_id = st.text_input("物品ID", key="item_checkout_inputid").strip()
...
user_class = col1.text_input("班级")
user_phone = col2.text_input("手机号")
...
if st.button("确认领用"):
    ...
    #     st.error("内部人员核验密码错误！")
        ...
        get_or_create_user(user, user_name, user_class, user_phone)  # user_source
        ...
        if item_found is None:
            ...
            # 更新物品状态
            items[item_found]["username"] = user 
            items[item_found]["name"] = user_name 
            items[item_found]["user_class"] = user_class 
            items[item_found]["user_phone"] = user_phone
            ...
            st.success(f"{user_name} (ID:{user}) 成功领用 {items[item_found]['item_name']}")